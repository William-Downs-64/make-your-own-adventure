...
//select scenario
    ...
    echo "<form method='post' action='multi.php?area=1&old=0'>
        <select name='username' id='username' class='btn btn-info btn-lg' value='$username'>
            <option>WieRD</option>
...
<body>
    ...
    <!-- <a href="multi.php?area=1&old=0" class="btn btn-secondary btn-lg display-2">Restart</a> -->

    <h1 class="text-center"><u>Make your own Adventure</u></h1>

    <div class="container p-3 mt-4 border main-box">
        <div class="row">
        <?php
            if(array_key_exists('area', $_GET)) {
                if($row) {
                    ...
                    if(str_contains($row[1], "You win!")) {
                        echo '<a href="multi.php?area=1&old=0" class="btn btn-info col">Congratulations!</a>';
                    }
                    else {
                        
                    //check die
                    if(str_contains($row[1], "You die.")) {
                        echo '<a href="multi.php?area=1&old=0" class="btn btn-danger col">Restart</a>';
                    }
                        
                    else {
                        echo "<br><a class='btn btn-primary col' href='multi.php?area=" . $row[3] . "&old=" . $_GET["area"] . "&choice=1'>" . $row[2] . "</a>";
                        
                        //check second choice
                        if($row[4] != "") {
                            echo "<br><a class='btn btn-warning col' href='multi.php?area=" . $row[5] . "&old=" . $_GET["area"] . "&choice=2'>" . $row[4] . "</a>";
                        }
                    }

                }}
                //area not in database yet
                else {
                    if(!array_key_exists('choice', $_GET)) {
                        $_GET['choice'] = 0;
                    }

                    echo "<h3 class='text-center'>New Path found!</h3>";
                    ...
                    //new path form
                    echo "<form action='multi.php?area=1&old=" . $_GET['old'] . "&choice=" . $_GET['choice'] . "' method='post'>
