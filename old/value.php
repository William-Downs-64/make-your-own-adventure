...
// Create connection
$conn = new mysqli("localhost", "root", "", "cyoam");
...
//select scenario
    ...
    echo "<form method='post' action='value.php?area=1&old=0'>
...
<body>
    ...
    <!-- <a href="value.php?area=1&old=0" class="btn btn-secondary btn-lg display-2">Restart</a> -->
    ...
    <div class="container p-3 mt-4 border">
            ...
            if(array_key_exists('area', $_GET)) {
                if($row) {
                    ...
                    if(str_contains($row[1], "You win!")) {
                        echo '<a href="value.php?area=1&old=0" class="btn btn-info col">Congratulations!</a>';
                    }
                    else {
                        
                    //check die
                    if(str_contains($row[1], "You die.")) {
                        echo '<a href="value.php?area=1&old=0" class="btn btn-danger col">Restart</a>';
                    }
                        
                    else {
                        echo "<br><a class='btn btn-primary col' href='value.php?area=" . $row[3] . "&old=" . $_GET["area"] . "&choice=1'>" . $row[2] . "</a>";
                        
                        //check second choice
                        if($row[4] != "") {
                            echo "<br><a class='btn btn-warning col' href='value.php?area=" . $row[5] . "&old=" . $_GET["area"] . "&choice=2'>" . $row[4] . "</a>";
                        }

                        if($row[6] != "") {
                            echo "<br><a class='btn btn-secondary col' href='value.php?area=" . $row[7] . "&old=" . $_GET["area"] . "&choice=3'>" . $row[6] . "</a>";
                        }
                    }
                    ...
                    echo "New Path found!";
                    echo "<form action='value.php?area=1&old=" . $_GET['old'] . "&choice=" . $_GET['choice'] . "' method='post'>
                        <label for='description'>What happens?</label>
                        <textarea id='area' name='description' class='form-control'></textarea>

                        <label for='score'>Score Value?</label>
        ...
        <!-- <form action='value.php?area=1&old=0' method='post'>
